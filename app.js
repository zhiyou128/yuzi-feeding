...
function loadState() {
  try {
    ...
    base.observations = base.observations.map(o => { if (!o.ingredientId) { const hit = base.ingredients.find(i => i.name === o.ingredient); if (hit) o.ingredientId = hit.id; } return o; });
    return base;
...
function todayOverview(observingCount) {
  ...
  return `<section class="today-overview" aria-labelledby="overview-title"><h3 id="overview-title">今日概览</h3><div class="stats-strip">${stats.map(([icon, value, label]) => `<div class="stat stat-${icon}"><i aria-hidden="true">${icon === 'meal' ? '◌' : icon === 'leaf' ? '⌁' : icon === 'star' ? '★' : '●'}</i><div><strong>${value}</strong><span>${label}</span></div></div>`).join('')}</div></section>`;
}
...
function checkinObservation(ingredientId, day) {
  ...
  item.status = 'observing'; item.note = '排敏观察中';
  state.observations.push({ id: uid('obs'), ingredientId: ingredientId, ingredient: item.name, day: String(day), date: dateKey(), slot: 'morning', amount: '', skin: '', digestive: '', mood: '', abnormal: '', note: '', photo: '' });
  saveState(); render();
  if (day >= 3) { const item2 = state.ingredients.find(i => i.id === ingredientId); if (item2) { item2.status = 'tried'; item2.note = '已完成 3 天观察'; } saveState(); render(); showToast('三天打卡完成，已转为已吃过'); }
  else showToast(`第 ${day} 天已打卡`);
}
function saveObservation(form) { const data = Object.fromEntries(new FormData(form).entries()); const item = state.ingredients.find(i => i.id === form.dataset.ingredient); if (!item) return; item.status = 'observing'; item.note = '已有排敏观察记录'; state.observations.push({ ...data, ingredient: item.name, ingredientId: item.id, id: uid('obs') }); saveState(); closeModal(); render(); showToast('观察记录已保存'); }
